...
func (g *Gateway) discoverPeerIP(conn modules.PeerConn) error {
	...
	if err != nil {
		return modules.AddContext(err, "failed to split host from port")
	}
	e := types.NewEncoder(conn)
	modules.WriteString(host, e)
	return nil
}
	...
	for {
		...
		for addr, count := range addresses {
			if count > successfulResponses/2 {
				g.log.Println("INFO: ip successfully discovered using peers:", addr)
				return addr, nil
			}
		}
