...
func (g *Gateway) staticPingNode(addr modules.NetAddress) (err error) {
	...
	if err := d.Err(); err != nil {
		return modules.AddContext(err, "failed to read remote header")
	...
	modules.WriteString(modules.StopResponse, e)
...
func (g *Gateway) shareNodes(conn modules.PeerConn) error {
	...
	func() {
		...
		// desirable ones.
		for _, i := range frand.Perm(len(gnodes)) {
...
func (g *Gateway) requestNodes(conn modules.PeerConn) error {
	...
	num := modules.ReadUint64(d)
	if err := d.Err(); err != nil {
		return err
	}
	nodes := make([]modules.NetAddress, 0, num)
	for num > 0 {
		node := modules.NetAddress(d.ReadString())
		if err := d.Err(); err != nil {
			return err
		}
	...
	for _, node := range nodes {
		err := g.addNode(node)
		if err != nil && !modules.ContainsError(err, errNodeExists) && !modules.ContainsError(err, errOurAddress) {
...
func (g *Gateway) permanentNodePurger(closeChan chan struct{}) {
	...
	for {
		...
		g.mu.RUnlock()
		if modules.ContainsError(err, errNoNodes) {
...
func (g *Gateway) permanentNodeManager(closeChan chan struct{}) {
	...
	for {
		...
		g.mu.RUnlock()
		if modules.ContainsError(err, errNoPeers) {
