...
func (tp *TransactionPool) purge() {
	...
	tp.transactionHeights = make(map[types.TransactionID]uint64)
...
func (tp *TransactionPool) ProcessConsensusChange(cc modules.ConsensusChange) {
	...
	if modules.ContainsError(err, errNilRecentBlock) || cc.BlockHeight < tp.blockHeight {
		...
		tp.log.Println("CRITICAL: could not access recentID from tpool:", err)
	}
	...
	for _, block := range cc.RevertedBlocks {
		...
		for _, txn := range block.Transactions {
			err := tp.deleteTransaction(txn.ID())
			if err != nil {
	...
	for _, block := range cc.AppliedBlocks {
		...
		remaining := int(modules.BlockSizeLimit) - totalSize
		fees = append(fees, feeSummary{
