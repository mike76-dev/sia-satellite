...
// paymentAddressHandlerGET handles the GET /payment/address requests.
func (s *Server) paymentAddressHandlerGET(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	// Check for abuse.
	if err := s.checkAbuse(w, req); err != nil {
		return
	}
	...
	addr, err := s.accounts.GetAddress(acc)
	if err != nil {
	...
	s.writeJSON(w, PaymentAddressResponse{Address: addr})
}
