...
var (
	hostdbCmd = &cobra.Command{
		Use:   "hostdb",
		Short: "Interact with the manager's host database.",
...
// hostdbcmd is the handler for the command `satc hostdb`.
...
func hostdbcmd() {
	if !verbose {
		...
		for i, host := range offlineHosts {
			...
			// recent scans.
			price := host.StoragePrice.Mul(modules.BlockBytesPerMonthTerabyte)
			downloadBWPrice := host.StoragePrice.Mul64(modules.BytesPerTerabyte)
			fmt.Fprintf(w, "\t%v:\t%v\t%v\t%v\t%v\t%v\t%v\t%.3f\t%s\n", len(offlineHosts) - i, host.PublicKeyString, host.NetAddress, host.Version, modules.FilesizeUnits(host.RemainingStorage), price, downloadBWPrice, uptimeRatio, scanHistStr)
		}
		if err := w.Flush(); err != nil {
			die("failed to flush writer")
		}
		...
		for i, host := range inactiveHosts {
			...
			downloadBWPrice := host.DownloadBandwidthPrice.Mul64(modules.BytesPerTerabyte)
		...
		for i, host := range activeHosts {
			...
			collateral := host.Collateral.Mul(modules.BlockBytesPerMonthTerabyte)
			downloadBWPrice := host.DownloadBandwidthPrice.Mul64(modules.BytesPerTerabyte)
			fmt.Fprintf(w, "\t%v:\t%v\t%v\t%v\t%12.6g\t%v\t%v\t%v\t%v\t%v\t%.3f\t%s\n", len(activeHosts) - i, host.PublicKeyString, host.NetAddress, host.Version, score, modules.FilesizeUnits(host.RemainingStorage), host.ContractPrice, price, collateral, downloadBWPrice, uptimeRatio, scanHistStr)
		}
		fmt.Fprintln(w, "\t\tPubkey\tAddress\tVersion\tScore\tRemaining Storage\tContract Fee\tPrice (/ TB / Month)\tCollateral (/ TB / Month)\tDownload Price (/TB)\tUptime\tRecent Scans")
		if err := w.Flush(); err != nil {
			die("failed to flush writer")
...
func hostdbviewcmd(pubkey string) {
	...
	fmt.Fprintln(w, "\n\t\tOffered Collateral (TB / Mo):\t", info.Entry.Collateral.Mul(modules.BlockBytesPerMonthTerabyte))
	fmt.Fprintln(w, "\t\tMax Collateral:\t", info.Entry.MaxCollateral)
	fmt.Fprintln(w, "\t\tContract Price:\t", info.Entry.ContractPrice)
	fmt.Fprintln(w, "\t\tBase RPC Price:\t", info.Entry.BaseRPCPrice)
	fmt.Fprintln(w, "\t\tSector Access Price:\t", info.Entry.SectorAccessPrice)
	fmt.Fprintln(w, "\t\tStorage Price (TB / Mo):\t", info.Entry.StoragePrice.Mul(modules.BlockBytesPerMonthTerabyte))
	fmt.Fprintln(w, "\t\tDownload Price (1 TB):\t", info.Entry.DownloadBandwidthPrice.Mul64(modules.BytesPerTerabyte))
	fmt.Fprintln(w, "\t\tUpload Price (1 TB):\t", info.Entry.UploadBandwidthPrice.Mul64(modules.BytesPerTerabyte))
	fmt.Fprintln(w, "\t\tUnlock Hash:\t", info.Entry.Address)
