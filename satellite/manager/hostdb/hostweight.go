...
const (
	...
	// that is more than 50% of what the manager would expect given the amount
	// of storage being used, the host switching to a scoring strategy which
	// less intensly favors adding more collateral. As long as the host has
	// provided sufficient skin-in-the-game, enormous amounts of extra
	// collateral are less important.
	//
	...
	// exponentiation is very high because the manager will already
	// intentionally avoid hosts that do not have many successful interactions,
	// meaning that the bad points do not rack up very quickly.
...
func (hdb *HostDB) basePriceAdjustments(entry smodules.HostDBEntry) float64 {
	...
	if baseRPCPrice.Cmp(maxBaseRPCPrice) > 0 {
		// hdb.staticLog.Printf("Host getting 0 score for BaseRPCPrice: Host %v, BaseRPCPrice %v, MaxBaseRPCPrice %v\n", entry.PublicKey.String(), baseRPCPrice.HumanString(), maxBaseRPCPrice.HumanString())
	...
	if sectorAccessPrice.Cmp(maxSectorAccessPrice) > 0 {
		// hdb.staticLog.Printf("Host getting 0 score for SectorAccessPrice: Host %v, SectorAccessPrice %v, MaxSectorAccessPrice %v\n", entry.PublicKey.String(), sectorAccessPrice.HumanString(), maxSectorAccessPrice.HumanString())
		return math.SmallestNonzeroFloat64
	}
...
func (hdb *HostDB) priceAdjustments(entry smodules.HostDBEntry, allowance modules.Allowance, txnFees types.Currency) float64 {
	...
	if err != nil {
		...
		// strictly not valuable given our allowance and its pricing. This is
		...
		if !strings.Contains(err.Error(), "exceeds funding") {
			...
			hdb.staticLog.Println(errors.AddContext(err, info))
		}
		return math.SmallestNonzeroFloat64
	}
...
func versionAdjustments(entry smodules.HostDBEntry) float64 {
	...
	if build.VersionCmp(entry.Version, "1.5.10") < 0 {
	...
	if build.VersionCmp(entry.Version, "1.5.9") < 0 {
		base = base * 0.99 // Slight penalty against slightly out of date hosts.
	}
	if build.VersionCmp(entry.Version, "1.5.8") < 0 {
		base = base * 0.99 // Slight penalty against slightly out of date hosts.
	}
...
func (hdb *HostDB) uptimeAdjustments(entry smodules.HostDBEntry) float64 {
	...
	for _, scan := range entry.ScanHistory[1:] {
		if recentTime.After(scan.Timestamp) {
			hdb.staticLog.Println("WARN: Host entry scan history not sorted.")
	...
	if uptime == 0 && downtime == 0 {
		hdb.staticLog.Println("CRITICAL: uptime and downtime are zero for this host, should have been caught in earlier logic")
		return math.SmallestNonzeroFloat64
	}
