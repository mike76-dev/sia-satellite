...
func (c *Contractor) managedNewContract(rpk types.SiaPublicKey, host smodules.HostDBEntry, contractFunding types.Currency, endHeight types.BlockHeight) (_ types.Currency, _ modules.RenterContract, err error) {
	...
	// Update the hostdb to include the new contract. TODO
...
func (c *Contractor) managedRenewContract(renewInstructions fileContractRenewal, blockHeight, endHeight types.BlockHeight) (fundsSpent types.Currency, err error) {
	// Check if we know this renter.
...
func (c *Contractor) threadedContractMaintenance() {
	...
	defer func() {
		if registerWalletLockedDuringMaintenance {
			c.staticAlerter.RegisterAlert(smodules.AlertIDWalletLockedDuringMaintenance, AlertMSGWalletLockedDuringMaintenance, smodules.ErrLockedWallet.Error(), smodules.SeverityWarning)
		} else {
			c.staticAlerter.UnregisterAlert(smodules.AlertIDWalletLockedDuringMaintenance)
		}
	}()

	// Perform general cleanup of the contracts. This includes archiving
	// contracts and other cleanup work. This should all happen before
	...
	// Check if all renters have anough contracts according to their allowances.
	c.mu.RLock()
	renters := c.renters
	...
	defer func() {
		...
		alertSeverity := smodules.SeverityError
		...
		if float64(numRenewFails) > math.Ceil(float64(numHosts) * MaxCriticalRenewFailThreshold) {
			alertSeverity = smodules.SeverityCritical
		}
		if renewErr != nil {
			c.log.Println("SEVERE", numRenewFails, float64(numHosts) * MaxCriticalRenewFailThreshold)
			c.log.Println("alert err: ", renewErr)
			c.staticAlerter.RegisterAlert(smodules.AlertIDRenterContractRenewalError, AlertMSGFailedContractRenewal, renewErr.Error(), smodules.AlertSeverity(alertSeverity))
		} else {
			c.staticAlerter.UnregisterAlert(smodules.AlertIDRenterContractRenewalError)
		}
	}()

	for _, renter := range renters {
		if renter.Allowance.Hosts <= 0 {
			continue
		}
		// Clear the slices for the garbage collection.
		renewSet = nil
		refreshSet = nil
		numHosts = numHosts + int(renter.Allowance.Hosts)
		...
		for _, contract := range c.staticContracts.ByRenter(renter.PublicKey) {
			...
			if !ok || !utility.GoodForRenew {
				if blockHeight - contract.StartHeight < types.BlocksPerWeek {
					c.log.Println("Contract did not last 1 week and is not being renewed", contract.ID)
				}
		...
		for _, renewal := range renewSet {
			...
			fundsSpent, err := c.managedRenewContract(renewal, blockHeight, renter.ContractEndHeight())
