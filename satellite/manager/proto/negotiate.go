...
func verifySettings(conn net.Conn, host modules.HostDBEntry) (modules.HostDBEntry, error) {
	...
	if host.PublicKey.Algorithm != types.SignatureEd25519 || len(host.PublicKey.Key) != crypto.PublicKeySize {
		fmt.Println("CRITICAL: HostDB did not filter out host with wrong signature algorithm:", host.PublicKey.Algorithm)
...
func verifyRecentRevision(conn net.Conn, contract *FileContract, hostVersion string) error {
	// Send contract ID.
...
func performSessionHandshake(conn net.Conn, hostPublicKey types.SiaPublicKey) (cipher.AEAD, modules.LoopChallengeRequest, error) {
	...
	if err != nil {
		fmt.Println("CRITICAL: Could not create cipher")
		return nil, modules.LoopChallengeRequest{}, err
	}
